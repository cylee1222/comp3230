...
int main(int argc, char* argv[])
{
    ...
    // The number of bytes of shared memory it requested;
    pid_t fpid = fork();
    ...
    if(fpid == 0){
      ...
      // The PID of process B and the signal it received.
      if(differ[2] > 0)
      ...
      // The comparison result: Compare the sum (differ[2]) with 0 to check if the 3rd argument (v2) is smaller or larger than or equal to the 1st argument (v1).
      shmdt(differ);
      exit(0);
      ...
      if(fpid1 < 0) exit(0);
        ...
        // The PID of the child process A and the signal it received.
        printf("Sum of differences: %d.\n", differ[2]);
        // The sum (differ[2]) of differences
        printf("Send a SIGCONT to process %d.\n\n", fpid);
        // The PID of process B who will receive the signal and the type of signal sent;
        shmdt(differ);
        kill(fpid, SIGCONT);
        exit(0);
        ...
        printf("Send a SIGCONT to process %d.\n\n", fpid1);
        // The child’s PID which will receive the signal and the type of the signal it sends;
        ...
        // The exited process’ PID and the number of exited child processes;
        shmdt(differ);
