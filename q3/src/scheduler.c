...
void *revisedSimpleRobotRoutine(void *arg) {
  ...
  while (!queueIsEmpty(task->jobQ)) {
    ...
    switch (jobID) {
    case SKELETON:
      sem_getvalue(&task->sem_A, &valA);
      sem_getvalue(&task->sem_B, &valB);
      sem_getvalue(&task->sem_C, &valC);
      if(valC > 0){
        if(type == 'C' && sem_trywait(&task->sem_C) == 0){
          queueDequeueFront(task->jobQ, &jobID);
#ifdef DEBUG
          debug_printf(__func__, "Robot%c[%d]: working on job %d...\n", 
              RobotTypeToChar(robot->robotType), robot->id, jobID);
          ...
          do{
            sem_getvalue(&task->sem, &cnt);
          }while(cnt == 0);
        }
      }else if(valB > 0){
        if(type == 'B' && sem_trywait(&task->sem_B) == 0){
          queueDequeueFront(task->jobQ, &jobID);
#ifdef DEBUG
          debug_printf(__func__, "Robot%c[%d]: working on job %d...\n", 
              RobotTypeToChar(robot->robotType), robot->id, jobID);
        ...
        if(type == 'A' && sem_trywait(&task->sem_A) == 0){
          queueDequeueFront(task->jobQ, &jobID);
#ifdef DEBUG
          debug_printf(__func__, "Robot%c[%d]: working on job %d...\n", 
              RobotTypeToChar(robot->robotType), robot->id, jobID);
